!------------------------------------------------------------------
! This program reads files with parameters of electron particles that collided with a given boundary object.
! Then this program saves for each segment of the boundary object: 
! a file with the particle flux and particle energy flux as function of coordinate along the segment, and
! a file with the energy spectrum of collided electrons (number of particles in energy bins) as function of coordinate along the segment.
!
! During a simulation, the EDPIC-2D code will save, if requested, all electrons collided with a given boundary object into a set of files.
! Each file contains particles collided with the boundary object during time interval between two consecutive snapshots.
!
! For example, file _0003_e_collided_with_bo_01.bin, contains parameters of all electron particles that collided with 
...
! a token which contains the number of segment of boundary object that the particle collided with and the tag of the particle
...
! The input of the user is saved into a control text file dtp_e_wall_coll.dat. The user may edit this file before running the program next time.
...
! 
program get_energy_spectrum_of_e_collided_with_walls
  ...
  type cell_data
     real x_m
     real y_m
     real particle_number_flux
     real particle_energy_flux
  ...
  character(44) Javg_filename ! avg_current_of_e_collided_with_bo_NN_vst.dat
!                             ! ----*----I----*----I----*----I----*----I----

  integer snap
!                                 ! ----x----I----x----I----x----I-
  character(31) filename_ecoll    ! _NNNN_e_collided_with_bo_NN.bin
  ...
  real avgJ_A   ! average current of electrons (time average flux integrated over the surface, in Amperes, non-negative)

  integer rec_count     ! counter of different time moments (records)
  integer n_parts       ! number of particles
  integer skip_count    ! counter of particles with energies beyond limits assignet to teh energy distribution function

  real minimal_e_energy_eV
  real maximal_e_energy_eV
...
!                                         ! ----*----I----*----I----*----I-
  character(31) filename_segefluxes       ! _NNNN_bo_NN_seg_NN_e_fluxes.asc
!                                         ! ----*----I----*----I----*----I----*----I
  character(40) filename_segespectr       ! _NNNN_bo_NN_seg_NN_e_energy_spectrum.asc
  ...
  inquire (file = 'dtp_e_wall_coll.dat', exist = exists)

  if (exists) then

     open (9,file = 'dtp_e_wall_coll.dat')
     ...
     read (9, '(A1)') buf  ! # the minimal energy for electron energy spectrum (eV)
     read (9, *) w_min_eV
     read (9, '(A1)') buf  ! # the maximal energy for electron energy spectrum (eV)
     read (9, *) w_max_eV
     read (9, '(A1)') buf  ! # the energy bin size for electron energy spectrum (eV)
     read (9, *) dw_eV
    close (9, status = 'keep')

  else

     print '("file dtp_e_wall_coll.dat not found, do manual input...")'
     print '("### inputs will be saved into file dtp_e_wall_coll.dat, edit this file as necessary for future use or remove ###")'
     ...
     print *, "enter the minimal energy for electron energy spectrum (eV)"
     read *, w_min_eV

     print *, "enter the maximal energy for electron energy spectrum (eV)"
     read *, w_max_eV

     print *, "enter the energy bin size for electron energy spectrum (eV)"
     read *, dw_eV
     ...
     write (9, '("# the minimal energy for electron energy spectrum (eV)")')
     write (9, '(2x,f10.3)') w_min_eV
     write (9, '("# the maximal energy for electron energy spectrum (eV)")')
     write (9, '(2x,f10.3)') w_max_eV
     write (9, '("# the energy bin size for electron energy spectrum (eV)")')
     write (9, '(2x,f10.3)') dw_eV
     close (9, status = 'keep')

     print '("created file dtp_e_wall_coll.dat")'
...
!  end do
  ...
  if (save_avg_currents_vs_time_flag.gt.0) then
     Javg_filename = 'avg_current_of_e_collided_with_bo_NN_vst.dat'
!                     ----*----I----*----I----*----I----*----I----
     Javg_filename(35:36) = convert_int_to_txt_string(nbo, 2)
...
!                    ! ----x----I----x----I----x----I-
     filename_ecoll = '_NNNN_e_collided_with_bo_NN.bin'
     filename_ecoll(2:5) = convert_int_to_txt_string(snap, 4)
     filename_ecoll(26:27) = convert_int_to_txt_string(nbo, 2)
     inquire (file = filename_ecoll, exist = exists)

     if (.not.exists) then
        print '("file ",A31," not found, skip")', filename_ecoll
        cycle
     end if

     open (19, file = filename_ecoll, access = 'stream', form = 'unformatted')
     print '("processing file ",A31)', filename_ecoll
     ...
     energy_factor_eV = 0.5_8 * m_e_kg * V_scale_ms * V_scale_ms / e_Cl
...
! vertical segment
           ...
           do isb = segment(n)%start_indx, segment(n)%end_indx
              ...
              allocate(segment(n)%spatial_box(isb)%N_of_parts_in_energy_box(1:N_of_energy_boxes), stat = alloc_err)
           end do
...
! read particle data
     ...
     n_parts = 0
     skip_count = 0
     minimal_e_energy_eV =  1.0e7  ! initialize with values that make no sense
     maximal_e_energy_eV = -1.0e7  !
...
! EOF
        ...
        if (ios.ne.0) then
! this is an error
           print '("error-2 but we try to save what we already have")'
           exit
        end if
        ...
        if (print_record_details_flag.gt.0) print '(" record ",i6," time ",f12.5," ns ",i6," e particles")', rec_count, t_record_ns, N_parts_in_record
        ...
        do k = 1, N_parts_in_record
           ...
           segment(n)%spatial_box(isb)%particle_number_flux = segment(n)%spatial_box(isb)%particle_number_flux + 1.0

           w_eV = vx * vx + vy * vy + vz * vz  ! here it is not in eV yet

           segment(n)%spatial_box(isb)%particle_energy_flux = segment(n)%spatial_box(isb)%particle_energy_flux + w_eV

           w_eV = w_eV * energy_factor_eV                     ! now it is in eV

           if (w_eV.lt.minimal_e_energy_eV) minimal_e_energy_eV = w_eV
           if (w_eV.gt.maximal_e_energy_eV) maximal_e_energy_eV = w_eV

           if ((w_eV.ge.w_min_eV).and.(w_eV.le.w_max_eV)) then
              iwb = 1 + (w_eV - w_min_eV) / dw_eV
              iwb=max(1, min(iwb, N_of_energy_boxes))
              segment(n)%spatial_box(isb)%N_of_parts_in_energy_box(iwb) = segment(n)%spatial_box(isb)%N_of_parts_in_energy_box(iwb) + 1
              if (print_each_particle_flag.ne.0) then
                 print '("part ",i6," token ",i6," seg ",i2," tag ",i3," coll.coord ",f10.4," coll.indx ",i5," vx/vy/vz ",e12.5," ",e12.5," ",e12.5,"   W ",f10.3," eV, energy box ",i6)', &
                      & k, token, n, tag, coll_coord, isb, vx, vy, vz, w_eV, iwb
              end if
           else          
              skip_count = skip_count + 1
              if (print_each_particle_flag.ne.0) then
                 print '("part ",i6," token ",i6," seg ",i2," tag ",i3," coll.coord ",f10.4," coll.indx ",i5," vx/vy/vz ",e12.5," ",e12.5," ",e12.5,"   W ",f10.3," eV")', &
                      & k, token, n, tag, coll_coord, isb, vx, vy, vz, w_eV
              end if
           end if
     ...
     print '("actual energy range from ",f10.3," eV to ",f10.3," eV")', minimal_e_energy_eV, maximal_e_energy_eV
...
!     print '("end time = ",f12.5," ns")', t_end_ns
     ...
     print '("finished reading file ",A31)', filename_ecoll

     do n = 1, N_of_segments

!---------------------- particle number/energy fluxes to the wall ---------------------

        filename_segefluxes = '_NNNN_bo_NN_seg_NN_e_fluxes.asc'
!                              ----*----I----*----I----*----I-
        filename_segefluxes(2:5)   = convert_int_to_txt_string(snap, 4)
        filename_segefluxes(10:11) = convert_int_to_txt_string(nbo, 2)
        filename_segefluxes(17:18) = convert_int_to_txt_string(n, 2)
        
        open (9, file = filename_segefluxes)

        write (9, '("# beginning time ",f12.5," ns, end time ",f12.5," ns, number of macroparticles ",i6)') t_begin_ns, t_end_ns, n_parts
...
! vertical segment
           do isb = segment(n)%start_indx, segment(n)%end_indx
              write (9, '(2x,f8.1,2x,f8.1,2x,f12.7,2x,f12.7,2x,e12.5,2x,e12.5)') &
                   ...
                   & segment(n)%spatial_box(isb)%particle_number_flux * number_flux_factor_mm2ns, &                     ! 5
                   & segment(n)%spatial_box(isb)%particle_energy_flux * number_flux_factor_mm2ns * energy_factor_eV     ! 6
           end do
        else
! horizontal segment
           do isb = segment(n)%start_indx, segment(n)%end_indx
        ...
        print '("created file ",A31)', filename_segefluxes

!---------------------- energy spectrum of particles collided with the wall ---------------------

        filename_segespectr = '_NNNN_bo_NN_seg_NN_e_energy_spectrum.asc'
!                              ----*----I----*----I----*----I----*----I
        filename_segespectr(2:5)   = convert_int_to_txt_string(snap, 4)
        filename_segespectr(10:11) = convert_int_to_txt_string(nbo, 2)
        filename_segespectr(17:18) = convert_int_to_txt_string(n, 2)
        
        open (9, file = filename_segespectr)
           ...
           do isb = segment(n)%start_indx, segment(n)%end_indx
              do iwb = 1, N_of_energy_boxes
                 write (9, '(2x,f8.1,2x,f8.1,2x,f12.7,2x,f12.7,2x,i6,2x,f10.4,2x,i6)') &
                      ...
                      & segment(n)%spatial_box(isb)%N_of_parts_in_energy_box(iwb)         ! 7
              end do
              write (9, '(" ")')
           end do
        else
! horizontal segment
        ...
        print '("created file ",A40)', filename_segespectr
...
! which can be compared with history_bo_NN_avg.dat
     ...
     if (save_avg_currents_vs_time_flag.gt.0) then
        ...
        do n = 1, N_of_segments
           do isb = segment(n)%start_indx, segment(n)%end_indx
              avgJ_A = avgJ_A + segment(n)%spatial_box(isb)%particle_number_flux
           end do
        end do
        avgJ_A = avgJ_A * (1.0d9 * e_Cl * N_scale_part_m3 * delta_x_m * delta_x_m / (t_end_ns - t_begin_ns))   ! gives current in Amperes for single charged ions
        write (10, '(2x,f14.6,2x,f14.6,2x,e12.5)') t_begin_ns, t_end_ns, avgJ_A
     end if
...
! cleanup
  ...
  if (save_avg_currents_vs_time_flag.gt.0) then
     ...
     print '("file ",A44," is ready")', Javg_filename
  end if

end program get_energy_spectrum_of_e_collided_with_walls
