...
SUBROUTINE ADVANCE_ELECTRONS_PLUS
  ...
  collect_electron_moments_now = .FALSE.
  ...
  IF (collect_electron_moments_now) THEN
...
END SUBROUTINE ADVANCE_ELECTRONS_PLUS

!---------------------------------------
!
SUBROUTINE ADVANCE_ELECTRONS_AND_CALCULATE_MOMENTS_2D
...
!### electron velocities updV* correspond to the same time as the coordinates before update ###
     rvx = REAL(updVX)
     rvy = REAL(updVY)
     rvz = REAL(updVZ)
...
! now cluster masters exchange information about densities in overlapping nodes
  ...
  CALL SYNCHRONIZE_MOMENTS_IN_OVERLAP_NODES   !### NEW :: only inner nodes are synchronized now, external nodes must be discarded
...
END SUBROUTINE ADVANCE_ELECTRONS_AND_CALCULATE_MOMENTS_2D

!---------------------------------------
!
SUBROUTINE ADVANCE_ELECTRONS_AND_CALCULATE_MOMENTS_PROBES
...
!---------------------------------->>>
  IF (N_of_probes_cluster.GT.0) THEN
     bufsize = 13 * N_of_probes_cluster  ! {N,VX,VY,VZ,WX,WY,WZ,VXVY,VXVZ,VYVZ,QX,QY,QZ}
...
! the probe is in the right top corner of the cell containing the particle
        ...
        rvx = REAL(0.5_8 * (electron(k)%VX + updVX))
        rvy = REAL(0.5_8 * (electron(k)%VY + updVY))
        rvz = REAL(0.5_8 * (electron(k)%VZ + updVZ))
...
END SUBROUTINE ADVANCE_ELECTRONS_AND_CALCULATE_MOMENTS_PROBES
