  ...
  DO j = c_indx_y_min, c_indx_y_max
     DO i = c_indx_x_min, c_indx_x_max
        IF (cs_N(i,j).GT.0.0) THEN
           ...
           cs_TX(i, j) = 2.0 * cs_WX(i, j) - REAL(m_e_kg / e_Cl) * cs_VX(i, j)**2
           cs_TY(i, j) = 2.0 * cs_WY(i, j) - REAL(m_e_kg / e_Cl) * cs_VY(i, j)**2
           cs_TZ(i, j) = 2.0 * cs_WZ(i, j) - REAL(m_e_kg / e_Cl) * cs_VZ(i, j)**2
